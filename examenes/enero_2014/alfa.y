...
%type <atributos> compare_until_less
%type <atributos> compare_until_equal
%type <atributos> compare_until_greater
%type <atributos> compare_exp
...
condicional: if_exp TOK_LLAVEIZQUIERDA sentencias TOK_LLAVEDERECHA                                                            
           ...
           | compare_until_greater sentencias TOK_LLAVEDERECHA
                {
                    compare_with_fin(out, $1.etiqueta);
                }
           ;

compare_until_less: compare_exp TOK_LLAVEIZQUIERDA compare_less 
                    {
                        compare_less(out, $1.etiqueta);
                        $$.etiqueta = $1.etiqueta;
                    }
                ;
compare_until_equal: compare_until_less sentencias compare_equal
                    {
                        compare_any_fin(out, $1.etiqueta);
                        compare_equal(out, $1.etiqueta);
                        $$.etiqueta = $1.etiqueta;
                    }
                ;
compare_until_greater: compare_until_equal sentencias compare_greater 
                    {
                        compare_any_fin(out, $1.etiqueta);
                        compare_greater(out, $1.etiqueta);
                        $$.etiqueta = $1.etiqueta;
                    }
                ;

compare_exp: TOK_COMPARE exp TOK_WITH exp 
                {
                    /* Comprobacion semantica */
                    if ($2.tipo != INT || $4.tipo != INT) {
                        print_error_semantico(ERROR_COMPARE_TIPO, NULL);
                        return 1;
                    }
                    ...
                    etiq_condicional += 1;
                    /* GENERACION */
                    compare_with(out, $2.es_direccion, $4.es_direccion, $$.etiqueta);
                }
            ;

compare_less: TOK_LESS
                {
...
void print_error_semantico(int tipo_error_semantico, char* nombre) {
    ...
    switch(tipo_error_semantico) {
        ...
        case ERROR_COMPARE_TIPO:
            printf("****Error semantico en lin <%ld>: Se esperaba una expresion de tipo entero.", yylin);
            break;
    }
    printf("\n");
}
